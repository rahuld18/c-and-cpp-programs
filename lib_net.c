...
void deletebooks()    //function that delete items from file fp
{
    ...
    if(findbook=='t' )
    {
        ...
        fclose(fp);
        remove("Bibe.dat");
        rename("test.dat","Bibe.dat"); //copy all item from temporary file to fp except that
        fp=fopen("Bibe.dat","rb+");    //we want to delete
        if(findbook=='t')
        {
            gotoxy(10,10);
...
void searchbooks()
{
    ...
    gotoxy( 15,20);
    printf("Enter Your Choice");
    fp=fopen("Bibe.dat","rb+"); //open file for reading propose
